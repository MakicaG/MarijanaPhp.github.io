...
<head>
    ...
    <title>Mystic coffee-order detail</title>
</head>
<body>
...
<div class="title">
    ...
    <div class="box-container">
        ...
        if ($select_orders->rowCount() > 0) {
            while ($fetch_order = $select_orders->fetch(PDO::FETCH_ASSOC)) {
                ...
                if ($select_product->rowCount() > 0) {
                    while (
                        $fetch_product = $select_product->fetch(
                            PDO::FETCH_ASSOC
                        )
                    ) {
        ...
        <div class="box">
            <div class="col">
                <p class="title"><i class='bx bxs-calendar'></i> 
                <?= $fetch_order["data"] ?></p>
                <img src="assets/img/<?= $fetch_product[
                    "image"
                ] ?>" alt="img" class="image"/>
                <p class="price"><?= $fetch_product[
                    "price"
                ] ?> x <?= $fetch_order["qty"] ?></p>
                <h3 class="name">
                <?= $fetch_product["name"] ?>
            ...
            <div class="col">
                ...
                <p class="user"><i class='bx bx-user'></i><?= $fetch_order[
                    "name"
                ] ?></p>
                <p class="user"><i class='bx bx-phone'></i><?= $fetch_order[
                    "number"
                ] ?></p>
                <p class="user"><i class='bx bx-envelope' ></i><?= $fetch_order[
                    "email"
                ] ?></p>
                <p class="user"><i class='bx bx-map-pin'></i><?= $fetch_order[
                    "address"
                ] ?></p>
                <p class="title">
                    status
