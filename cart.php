...
if (isset($_POST["empty_cart"])) {
    $verify_empty_item = $_POST["empty_cart"];
    $verify_empty_item->execute(["uder_id"]);
...
<head>
    ...
    <title>Mystic coffee-cart</title>
</head>
<body>
    ...
    while ($fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($select_products->rowCount() > 0) {
            ...
            <form action="" method="post" class="box">
                <input type="hidden" name="cart_id" value="<?= $fetch_cart[
                    "id"
                ] ?>">
                <img src="assets/img/<?= $fetch_products[
                    "image"
                ] ?>"  alt="prImg"/ class="img">
                <h3 class="name"><?= $fetch_products["name"] ?></h3>
                <div class="flex">
                    <p class="price">price  $<?= $fetch_products["price"] ?></p>
                    <input type="number" name="qty" required min="1" value="<?= $fetch_cart[
                        "qty"
                    ] ?>" max="99" maxlength="2" class="qty">
                    <button type="submit" name="update_cart" class="bx bxs-edit fa-edit"></button>
                </div>
                        <p class="sub-total">sub total: <span>$<?= $sub_total =
                            $fetch_cart["qty"] *
